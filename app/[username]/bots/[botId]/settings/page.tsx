...
export default function BotSettingsPage() {
  ...
  const fetchSettings = async () => {
    ...
    setSettings(data.settings || { prefix: '!', bot_name: 'Asuma Bot', auto_read: true, auto_typing: false, auto_read_sw: false, anti_call: false, anti_spam: true, custom_reply: '', welcome_message: '', default_group: '', default_channel: '', response_wait: '⏳ Tunggu sebentar...', response_error: '❌ Terjadi kesalahan!', response_limit: 'Limit kamu habis!', response_success: '✅ Berhasil!', response_cooldown: 'Tunggu sebentar...' })
    setLoading(false)
  }
  ...
  const save = async () => {
    setSaving(true)
    await fetch(`/api/bots/${botId}/settings`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(settings) })
    setSaving(false)
    setToast('Berhasil disimpan!')
    setTimeout(() => setToast(''), 3000)
  }

  if (loading) return <div className="flex items-center justify-center min-h-[60vh]"><Loader2 size={32} className="animate-spin text-emerald-400" /></div>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto">
      <Link href={`/${username}/bots/${botId}`} className="flex items-center gap-2 text-gray-400 hover:text-white mb-6 transition"><ArrowLeft size={20} /><span className="text-sm">Kembali</span></Link>

      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Settings Bot</h1>
        <button onClick={save} disabled={saving} className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white px-4 py-2 rounded-xl transition text-sm font-medium">{saving ? <Loader2 size={16} className="animate-spin" /> : <Save size={16} />} Simpan</button>
      </div>

      {toast && <div className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 rounded-xl p-3 mb-4 text-sm text-center">{toast}</div>}
      ...
      <div className="space-y-4">
        {activeTab === 'basic' && (
          <>
            ...
            <SettingInput label="Default Group (ID)" value={settings.default_group} onChange={v => update('default_group', v)} placeholder="120363xxxxx@g.us" />
            <SettingInput label="Default Channel (ID)" value={settings.default_channel} onChange={v => update('default_channel', v)} placeholder="120363xxxxx@newsletter" />
          </>
        )}
